...
public class BallFollowDrive {
			...
			if (hasTarget) {
				if (isInCenter) {
					...
					Drivetrain.getInstance().drive(0, 0,
							RobotMap.AutoConstants.MAX_ANGULAR_SPEED_RADIANS_PER_SECOND * RobotMap.BallFollowMap.SLOW_SEARCH, false); // TODO
																													// Verify
																													// rotation
																													// speed
				}
			} else {
				Drivetrain.getInstance().drive(0, 0, RobotMap.AutoConstants.MAX_ANGULAR_SPEED_RADIANS_PER_SECOND * RobotMap.BallFollowMap.FAST_SEARCH,
	...
	private static boolean followTarget(double targetX, double targetY) {
		...
		Drivetrain.getInstance().drive(MathHelper.clampUnit(yMotion) * RobotMap.AutoConstants.MAX_SPEED_METERS_PER_SECOND / 2,
				MathHelper.clampUnit(xMotion) * RobotMap.AutoConstants.MAX_SPEED_METERS_PER_SECOND / 2,
				MathHelper.clampUnit(angleVel) * RobotMap.AutoConstants.MAX_ANGULAR_SPEED_RADIANS_PER_SECOND, false);
