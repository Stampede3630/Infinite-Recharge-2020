...
public class DriveTrain {
    ...
    private final ProfiledPIDController normal_turningPIDController = new ProfiledPIDController(1, 0.0, 0.02,
            new TrapezoidProfile.Constraints(kModuleMaxAngularVelocity, kModuleMaxAngularAcceleration));

    private final ProfiledPIDController bR_turningPIDController = new ProfiledPIDController(.5, 0.0, 0.02,
            new TrapezoidProfile.Constraints(kModuleMaxAngularVelocity, kModuleMaxAngularAcceleration));

    private final SwerveModule m_frontLeft = new SwerveModule(RobotMap.DRIVETRAIN_FRONT_LEFT_ANGLE_MOTOR,
            RobotMap.DRIVETRAIN_FRONT_LEFT_DRIVE_MOTOR, RobotMap.DRIVETRAIN_FRONT_LEFT_ANGLE_ENCODER,
            RobotMap.FRONT_LEFT_ANGLE_OFFSET, normal_turningPIDController);

    private final SwerveModule m_frontRight = new SwerveModule(
            RobotMap.DRIVETRAIN_FRONT_RIGHT_ANGLE_MOTOR,RobotMap.DRIVETRAIN_FRONT_RIGHT_DRIVE_MOTOR, RobotMap.DRIVETRAIN_FRONT_RIGHT_ANGLE_ENCODER,
            RobotMap.FRONT_RIGHT_ANGLE_OFFSET, normal_turningPIDController);
    private final SwerveModule m_backLeft = new SwerveModule(
            RobotMap.DRIVETRAIN_BACK_LEFT_ANGLE_MOTOR,RobotMap.DRIVETRAIN_BACK_LEFT_DRIVE_MOTOR,RobotMap.DRIVETRAIN_BACK_LEFT_ANGLE_ENCODER,
            RobotMap.BACK_LEFT_ANGLE_OFFSET, normal_turningPIDController);
    private final SwerveModule m_backRight = new SwerveModule(
            RobotMap.DRIVETRAIN_BACK_RIGHT_ANGLE_MOTOR,RobotMap.DRIVETRAIN_BACK_RIGHT_DRIVE_MOTOR,RobotMap.DRIVETRAIN_BACK_RIGHT_ANGLE_ENCODER,
            RobotMap.BACK_RIGHT_ANGLE_OFFSET, bR_turningPIDController);
    ...
    public Rotation2d getAngle() {
        ...
        // CHANGE
        return Rotation2d.fromDegrees(-RobotMap.m_gyro.getAngle());
    }
    ...
    public void driveWithJoystick(boolean fieldRelative) {
        ...
        var xSpeed = -RobotMap.controller.getY(Hand.kLeft) * RobotMap.kMaxSpeed;
        if (Math.abs(xSpeed) < (0.2 * RobotMap.kMaxSpeed)) {
          xSpeed = 0;
        }
        ...
        var ySpeed = -RobotMap.controller.getX(Hand.kLeft) * RobotMap.kMaxSpeed;
        if (Math.abs(ySpeed) < (0.2 * RobotMap.kMaxSpeed)) {
          ySpeed = 0;
        }
        ...
        var rot = -RobotMap.controller.getX(Hand.kRight) * RobotMap.kMaxAngularSpeed;
        if (Math.abs(rot) < (0.2 * RobotMap.kMaxAngularSpeed)) {
          rot = 0;
        }
